...
def build_super_images2(real_imgs, captions, cap_lens, ixtoword,
    ...
    for i in range(num):
        ...
        for j in range(num_attn):
            one_map = row_beforeNorm[j]
            #print(one_map)
            #print("DTYPE:", one_map.dtype)
            one_map *= 255
            #print(one_map.type)
            # one_map = one_map.astype(np.uint8).resize((256, 256))
            #.convert('RGB')

            #one_map = np.asarray(one_map).astype('float32')
            #print("DTYPE:", one_map.dtype, one_map.shape)
            #one_map = (one_map * 255).astype(np.uint8)
            #print("DTYPE:", one_map.dtype, one_map.shape)
            #one_map = one_map.reshape((256, 256))
            #print("SHAPY:", one_map.shape)
            #maybe_im = Image.fromarray((one_map * 255).astype(np.uint8).resize((256, 256))).convert("RGB")
            #print("MAYB", maybe_im.shape)

            #print('one_map', one_map)
            #one_map = np.reshape(one_map, (256, 256))
            #print('one_map2', one_map.shape)

            PIL_im = Image.fromarray(np.uint8(img))
            PIL_att = Image.fromarray(np.uint8(one_map))
            #print(PIL_att)
            #print(middle_pad.shape)
            merged = \
                Image.new('RGBA', (vis_size, vis_size), (0, 0, 0, 0))
